...
def label_smoothing(label_batch, smooth_range=label_smoothing_range):
    ...
    :return:
    """
    label_copy = label_batch.clone().detach()
    label_batch = label_batch * 0.9
    bsz = label_batch.size(0)
    expanded_label = torch.cat((torch.zeros(bsz, smooth_range), label_copy, torch.zeros(bsz, smooth_range)), dim=-1)
...
def label_smoothing_multi(labels_batch, smooth_range=label_smoothing_range):
    ...
    :return:
    """
    label_copy = labels_batch.clone().detach()  # (bsz, seq_l, num_classes)
    label_batch = labels_batch * 0.9    # (bsz, seq_l, num_classes)
    bsz, seq_l, num_classes = label_batch.size()
    expanded_label = torch.cat((torch.zeros(bsz, num_classes, smooth_range),
                                label_copy.permute([0, 2, 1]),
                                torch.zeros(bsz, num_classes, smooth_range)), dim=-1)    # (bsz, num_classes, seq_l + 2 * smooth_range)
    for k in range(smooth_range):
